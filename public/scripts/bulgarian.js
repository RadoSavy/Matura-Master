const appData = {
  ...
  leaderboardProgress:
    parseInt(localStorage.getItem('leaderboardProgress')) || 0,
...
const checkInterval = setInterval(() => {
  if (window.remoteBulgarianLessons && window.remoteBulgarianLessons.length > 0) {
    lessons = window.remoteBulgarianLessons;
    clearInterval(checkInterval);
    console.log(`✓ Bulgarian lessons loaded: ${lessons.length}`);
    initApp();
  }
}, 100);
...
setTimeout(() => {
  if (lessons.length === 0) {
    console.warn('No remote Bulgarian lessons loaded, using empty array');
    lessons = [];
    clearInterval(checkInterval);
    initApp();
  }
...
function initApp() {
  if (window.remoteBulgarianLessons && window.remoteBulgarianLessons.length > 0) {
    console.log('Using remote Bulgarian lessons from Firestore');
    finalLessons = window.remoteBulgarianLessons;
  } else if (lessons && lessons.length > 0) {
    finalLessons = lessons;
  }

  console.log('initApp: finalLessons length=', finalLessons.length);
  renderLessons();
  updateProgress();
  setupEventListeners();
  checkDailyStreak();
...
function renderLessons() {
  console.log('renderLessons called, finalLessons length=', finalLessons.length);
  const sections = [
    {
      id: 1,
      title: 'Раздел 1: Текстът в медийното и в естетическото общуване',
      lessons: finalLessons.slice(0, 4),
    },
    {
      id: 2,
      title: 'Раздел 2: Лексикология и фонетика',
      lessons: finalLessons.slice(4, 7),
    },
    { id: 3, title: 'Раздел 3: Морфология', lessons: finalLessons.slice(7, 10) },
    { id: 4, title: 'Раздел 4: Синтаксис', lessons: finalLessons.slice(11, 18) },
    {
      id: 5,
      title: 'Раздел 5: Устни и писмени ученически текстове',
      lessons: finalLessons.slice(18, 19),
    },
    { id: 6, title: 'Раздел 6: Приложения', lessons: finalLessons.slice(19, 21) },
  ];
...
function setupEventListeners() {
  if (closeModal) closeModal.addEventListener('click', closeLesson);
  if (nextQuestionBtn) nextQuestionBtn.addEventListener('click', nextQuestion);

  if (guidebookBtn) {
    guidebookBtn.addEventListener('click', () => {
      const guidebookLesson = finalLessons.find((l) => l.id === 1);
